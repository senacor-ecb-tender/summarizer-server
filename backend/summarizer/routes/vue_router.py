...
def vue_frontend(main___file__: str) -> APIRouter:
    ...
    def get_favicon():
        return _return_if_exists(pathlib.Path(main___file__).parent.parent / "templates" / "favicon.ico")

    @router.get("/{folder}/{fname}")
    def get_static(folder: str, fname: str):
        return _return_if_exists(pathlib.Path(main___file__).parent.parent / "templates" / folder / fname)
